...
def _get_pmcids_from_dois(dois):
    url = (
        "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pmc&term=%s"
...
def _get_query_url_from_pmcids(pmcids):
    return (
        "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pmc&id=%s"
        % "&id=".join(pmcids)
    )
...
def download_metadata(target_directory, dois=["10.1007/s12264-022-00834-9"]):
    ...
    local_filename = path.join(target_directory, url_path.split("/")[-1])
