...
from urllib3 import urlopen, urlparse, Request, HTTPError
from xml.etree.cElementTree import dump, ElementTree
from os import listdir, path, remove
from sys import stdin, stderr

from pmc import (
    _get_article_contrib_authors,
    _get_article_title,
    _get_article_abstract,
    _get_journal_title,
    _get_article_date,
    _get_article_url,
    _get_article_licensing,
    _get_article_copyright_holder,
    _get_supplementary_materials,
    _get_pmcid,
    _get_article_doi,
...
from pmc_doi import (
    _get_file_from_url,
    _get_query_url_from_pmcids,
    _get_file_from_pmcids,
)
...
def download_metadata(target_directory):
    ...
    for i, chunk in enumerate(chunker(pmcids, 365)):
        ...
        local_filename = path.join(target_directory, url_path.split("/")[-1] + str(i))
    ...
    for filename in listing:
        ...
        for tree in result_tree.iterfind("article"):
            ...
            (
                ...
                result["article-copyright-statement"],
            ) = _get_article_licensing(tree)
